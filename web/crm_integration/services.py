    ...
    for integration in CRMIntegration.objects.filter(is_active=True):
        try:
            ...
            try:
                ...
                log.status = 'error'
                log.error_message = f"Failed after retries: {str(e)}"
                log.save()
                
                # Создаем задачу на повторную синхронизацию через Celery
                from .tasks import retry_crm_sync
                retry_crm_sync.apply_async(
                    args=[request.id, integration.id],
                    countdown=300  # Повторная попытка через 5 минут
                )
